...
class model:
    ...
    def __init__(self, backend):
        ...
        self.v = view()
        # ↓ 新增：每个实例有自己的一份       这样可以直接在头文件 m = model() 后面建多个模型实例,不会再出现 共享 layer_list 导致的问题
        self.layer_list = []
        self.loss = None
        self.optimizer = None
    ...
    def predict(self, x_test, batch_size=10):       # 改
    # 1) 选切片方式 + 选拼接轴
        if x_test.ndim in (3, 4):
            n_sample = x_test.shape[0]                 # B
            take_batch = lambda b0, b1: x_test[b0:b1, ...]
            concat_axis = 0
        elif x_test.ndim == 2:
            n_sample = x_test.shape[1]                 # B(列)
            take_batch = lambda b0, b1: x_test[:, b0:b1]
            concat_axis = 1
        else:
            raise ValueError(f"predict: 不支持的输入形状 {x_test.shape}")

        # 2) 循环前向 + 正确拼接
        y_test = None
        for b_start in range(0, n_sample, batch_size):
            b_end = min(b_start + batch_size, n_sample)
            x_batch = take_batch(b_start, b_end)
            y_ = self.__forwardpass(x_batch)

            if y_test is None:
                y_test = y_
            else:
                y_test = np.concatenate((y_test, y_), axis=concat_axis)
